...
class Traceroute(ICMPPing):
    ...
    def sendIcmpProbesAndCollectResponses(self, ttl):
        ...
        for sequence in range(3):  # Send 3 probes
            # Send ICMP probe
            packetID = random.randint(1, 65535)
            timeSent = self.sendOnePing(self.dstAddress, packetID, sequence, ttl, dataLength=52)
            ...
            icmpType, responseTTL = self.parseICMPTracerouteResponse(trReplyPacket)

            # Check if the destination is reached
            if icmpType == 0 and self.dstAddress == hopAddr:  # Echo Reply (Type 0)
                self.isDestinationReached = True

            # Record RTT and hop address if response matches
    ...
    def parseICMPTracerouteResponse(self, trReplyPacket):
        ...
        ip_header = struct.unpack("!BBHHHBBH4s4s", trReplyPacket[:20])
        ip_header_len = (ip_header[0] & 0x0F) * 4  # Header length in bytes

        # Parse the ICMP header (following the IP header)
        icmp_header = trReplyPacket[ip_header_len:ip_header_len + 8]
        icmpType, code, checksum, identifier, sequence = struct.unpack("!BBHHH", icmp_header)

        return icmpType, ip_header[5]  # Return ICMP Type and TTL      
...
class MultiThreadedTraceRoute(Traceroute):
    ...
    def __init__(self, args):
        ...
        for ttl, hop_addrs, rtts in self.results:
            pkt_keys = sorted(rtts.keys())
            self.printMultipleResults(ttl, pkt_keys, hop_addrs, rtts)


    # TODO✅: Remove this once this method is implemented               
    ...
    def send_probes(self):
        ...
        while ttl <= MAX_TTL and not self.isDestinationReached:
            # Data containers for this TTL
            hop_addrs = {}
            rtts = {}
            pkt_keys = []

            for sequence in range(3):  # Send three probes per TTL
                if self.isDestinationReached:
                    break
                packetID = random.randint(1, 65535)

                if args.protocol == "icmp":
                    # Send ICMP probe and record time sent
                    timeSent = self.sendOnePing(self.dstAddress, packetID, sequence, ttl, dataLength=52)
                elif args.protocol == "udp":
                    # Send UDP probe and record time sent
                    dstPort = 33434 + sequence
                    timeSent = self.sendOneUdpProbe(self.dstAddress, dstPort, ttl, dataLength=52)
                else:
                    print(f"Invalid protocol: {args.protocol}")
                    return

                pkt_keys.append(sequence)  # Record sequence number
                rtts[sequence] = timeSent  # Record time sent for RTT calculation

                # Small delay between probes
                time.sleep(0.05)

            # Add this TTL's data to shared results
            with self.lock:
                self.results.append((ttl, hop_addrs, rtts))

            ttl += 1

        # Notify the receiving thread that all probes are sent
    ...
    def receive_responses(self):
        ...
        while not self.send_complete.is_set():
            try:
                ...
                responsePacket, hopAddr, timeReceived = self.receiveOneTraceRouteResponse()

                if responsePacket is None:
                    # No response within the timeout
                    continue

                # Process the response based on the protocol
                if args.protocol == "icmp":
                    icmpType, _ = self.parseICMPTracerouteResponse(responsePacket)
                    if icmpType == 0 and hopAddr == self.dstAddress:  # Echo Reply
                        self.isDestinationReached = True
                        self.send_complete.set()  # Signal to stop sending further probes
                        break
                elif args.protocol == "udp":
                    dstPort, icmpType = self.parseUDPTracerouteResponse(responsePacket)
                    if icmpType == 3 and hopAddr == self.dstAddress:  # Destination Unreachable
                        self.isDestinationReached = True
                        self.send_complete.set()
                        break

                # Update the results for the hop
                with self.lock:
                    for ttl, hop_addrs, rtts in self.results:
                        # Validate response and skip duplicates
                        if hopAddr in hop_addrs.values():
                            continue
                        for seq, timeSent in rtts.items():
                            if timeSent is None or seq not in rtts:
                                continue  # Skip mismatched responses
                            rtt = timeReceived - timeSent
                            hop_addrs[seq] = hopAddr
                            rtts[seq] = rtt
            except socket.timeout:
                # Skip if no response is received within the timeout
                continue    
...
class Proxy(NetworkApplication):
    def __init__(self, args):
        print('Web Proxy starting on port:', args.port)
    ...
    def handle_request(self, client_socket):
        try:
            ...
            request = client_socket.recv(MAX_DATA_RECV).decode()
            if not request:
                return
            
            # Parse the requested URL from the GET request
            request_line = request.split('\n')[0]
            url = request_line.split(' ')[1]
            
            # Convert the URL into a safe filename for caching
            cache_key = url.replace("/", "_")
            cache_path = os.path.join(self.cache_dir, cache_key)

            # Serve from cache if the file exists
